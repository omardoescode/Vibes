...
public class MessageHandler {
    ...
    private final List<MessageDeliveryStrategy> deliveryStrategies;
    ...
    @Transactional
    public void handle(MessagePayload payload, Principal principal, boolean isMedia) throws Exception {
        UUID senderId = UUID.fromString(principal.getName());
        payload.setSenderId(senderId.toString());

        Message saved = isMedia 
                ? messageService.processAndSaveMediaMessage(payload)
                : messageService.processAndSaveTextMessage(payload);
